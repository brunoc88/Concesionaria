...
exports.editarVehiculoForm = async (req, res) => {
    try {
        const { id } = req.params
        const vehiculo = await Vehiculo.findByPk(id)
        return res.status(200).render('vehiculo/editar', {
            vehiculo
        })
    } catch (error) {
        console.log(`Hubo un error: ${error}`)
...
exports.editarVehiculo = async (req, res) => {
    try {
        ...
        if (Object.keys(cambios).length === 0) {
            ...
            return res.status(400).render('vehiculo/editar', {
        ...
        if (duplicado) {
            vehiculo.idVehiculo = id;
            return res.status(409).render('vehiculo/editar', {
        ...
        req.session.message = `Vehículo: ${vehiculo.marca} ${vehiculo.modelo} ${vehiculo.anio}  actualizado!`
...
const chekCambios = async (vehiculo, id) => {
    ...
    if (vehiculo.anio && parseInt(vehiculo.anio) !== vehiculoDB.anio) cambios.anio = parseInt(vehiculo.anio)
    if (vehiculo.precio && parseFloat(vehiculo.precio) !== vehiculoDB.precio) cambios.precio = parseFloat(vehiculo.precio)
    if (vehiculo.cantidad && parseInt(vehiculo.cantidad) !== vehiculoDB.cantidad) cambios.cantidad = parseInt(vehiculo.cantidad)
